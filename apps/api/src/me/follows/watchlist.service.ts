 ...
 * derived season averages, their recent scoring, and the user's own note.
 *
...
export interface WatchlistEntry {
  ...
  // The user's own scouting note. Null until they write one.
  note: string | null;
  followedAt: Date;
...
export class WatchlistService {
  ...
  async getWatchlist(userId: string, query: Record<string, unknown>): Promise<PagedResult<WatchlistEntry>> {
    ...
    const [follows, total] = await Promise.all([
      this.prisma.followedPlayer.findMany({
        where: { userId },
      ...
      this.prisma.followedPlayer.count({ where: { userId } }),
    ]);
  ...
  /**
   * Just the ids of the players this user follows.
   *
   * @param userId - the signed-in user, from the session.
   * @returns every followed player's id, unpaginated.
   * @remarks This exists so a follow button anywhere in the app can tell
   *          whether it is already on, without paging through the board and
   *          without one request per player. It is deliberately NOT a page:
   *          a partial answer would let a button on page two render as
   *          "not followed" when it is. The payload is one uuid per follow,
   *          and a user following even a whole league's worth of players is
   *          a few kilobytes, so the unbounded read is bounded in practice by
   *          how many players exist.
   */
  async getWatchedPlayerIds(userId: string): Promise<{ playerIds: string[] }> {
    const follows = await this.prisma.followedPlayer.findMany({
      where: { userId },
      select: { playerId: true },
      orderBy: { createdAt: "desc" },
    });
    return { playerIds: follows.map((follow) => follow.playerId) };
  }
